...
data <- read.csv("./data/D5_ValidationSet.csv", fileEncoding="UTF-8-BOM")
...
#------Output ---------------------------------------------------------------------- 
    ...
    #Apriori_D5_val = format(Apriori_rules,scientific=FALSE) 
    
    #refactor to dataframe
    Apriori_D5_val <- as(Apriori_rules,"data.frame")
    #split LHS from RHS
    
    Apriori_D5_val <- Apriori_D5_val %>%
      separate(rules, c("Antecedent", "Consequent"), "=>")
    head(Apriori_D5_val)
    
    #filter output by pvalue
    Apriori_D5_val <- filter(Apriori_D5_val, Fisher<0.5)
    
    #round column values to remove exponent numbers 
    Apriori_D5_val$Fisher <- round(Apriori_D5_val$Fisher, digits = 3)
    Apriori_D5_val$hlift <- round(Apriori_D5_val$hlift, digits = 3)
    Apriori_D5_val$hconf <- round(Apriori_D5_val$hconf, digits = 3)
    
    #All rules
    write.csv(Apriori_D5_val,"./Output/D5_rules_validationSet.csv", row.names = FALSE);
